...
class Missingdemandpoints(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        # Extract by expression_buildings
        alg_params = {
            'EXPRESSION': ' "theme"  =  \'Buildings\' ',
            'INPUT': parameters['cartograpgictext'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['ExtractByExpression_buildings'] = processing.run('native:extractbyexpression', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        # Extract by location_dpintersect
        alg_params = {
            'INPUT': parameters['demandpoints'],
            'INTERSECT': outputs['MergeVectorLayers_fnlland']['OUTPUT'],
            'PREDICATE': [0],  # intersect
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['ExtractByLocation_dpintersect'] = processing.run('native:extractbylocation', alg_params, context=context, feedback=feedback, is_child_algorithm=True)

        feedback.setCurrentStep(12)
        if feedback.isCanceled():
            return {}

        # Extract by location_dpdisjoint
        alg_params = {
            'INPUT': parameters['demandpoints'],
            'INTERSECT': outputs['MergeVectorLayers_fnlland']['OUTPUT'],
            'PREDICATE': [2],  # disjoint
        ...
        outputs['ExtractByLocation_dpdisjoint'] = processing.run('native:extractbylocation', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        alg_params = {
            ...
            'INPUT': outputs['ExtractByLocation_dpdisjoint']['OUTPUT'],
            'REFERENCE_LAYER': outputs['MergeVectorLayers_fnlland']['OUTPUT'],
            'TOLERANCE': 10,
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        ...
        # Merge vector layers_dp
        alg_params = {
            'CRS': QgsCoordinateReferenceSystem('EPSG:27700'),
            'LAYERS': [outputs['ExtractByLocation_dpintersect']['OUTPUT'],outputs['SnapGeometriesToLayer']['OUTPUT']],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['MergeVectorLayers_dp'] = processing.run('native:mergevectorlayers', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        alg_params = {
            ...
            'INTERSECT': outputs['MergeVectorLayers_dp']['OUTPUT'],
            'PREDICATE': [2],  # disjoint
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        ...
        alg_params = {
            'INPUT': outputs['ExtractByExpression_buildings']['OUTPUT'],
        ...
        # Field calculator_ponhomes
        alg_params = {
            'FIELD_LENGTH': 50,
            'FIELD_NAME': 'pon_homes',
            'FIELD_PRECISION': 0,
            'FIELD_TYPE': 2,  # Text (string)
            'FORMULA': '1',
            'INPUT': outputs['ExtractByLocation_missingdp']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['FieldCalculator_ponhomes'] = processing.run('native:fieldcalculator', alg_params, context=context, feedback=feedback, is_child_algorithm=True)

        feedback.setCurrentStep(18)
        if feedback.isCanceled():
            return {}

        # Field calculator_gistoolid
        alg_params = {
            'FIELD_LENGTH': 50,
            'FIELD_NAME': 'gistool_id',
            'FIELD_PRECISION': 0,
            'FIELD_TYPE': 2,  # Text (string)
            'FORMULA': "concat('-', @row_number +1)",
            'INPUT': outputs['FieldCalculator_ponhomes']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['FieldCalculator_gistoolid'] = processing.run('native:fieldcalculator', alg_params, context=context, feedback=feedback, is_child_algorithm=True)

        feedback.setCurrentStep(19)
        if feedback.isCanceled():
            return {}

        # Field calculator_bld_num
        alg_params = {
            'FIELD_LENGTH': 100,
            'FIELD_NAME': 'bld_num',
            'FIELD_PRECISION': 0,
            'FIELD_TYPE': 2,  # Text (string)
            'FORMULA': ' "textstring" ',
            'INPUT': outputs['FieldCalculator_gistoolid']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['FieldCalculator_bld_num'] = processing.run('native:fieldcalculator', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        alg_params = {
            'CRS': QgsCoordinateReferenceSystem('EPSG:27700'),
            'LAYERS': [parameters['demandpoints'],outputs['FieldCalculator_bld_num']['OUTPUT']],

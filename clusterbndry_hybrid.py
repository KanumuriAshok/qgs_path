...
class Clusterbndry_hybrid(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        alg_params = {
            'INPUT': outputs['PolygonsToLines_cllbndry']['OUTPUT'],
            'OVERLAY': outputs['Clip_lines_clbndry']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['Difference_lineleft'] = processing.run('native:difference', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        # Merge vector layers_recreatedclusterlines
        alg_params = {
            'CRS': QgsCoordinateReferenceSystem('EPSG:27700'),
            'LAYERS': [outputs['Difference_lineleft']['OUTPUT'],outputs['Simplify_clipped_bndry']['OUTPUT']],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['MergeVectorLayers_recreatedclusterlines'] = processing.run('native:mergevectorlayers', alg_params, context=context, feedback=feedback, is_child_algorithm=True)

        feedback.setCurrentStep(37)
        if feedback.isCanceled():
            return {}

        # Snap geometries to layer_fnlclusterlines
        alg_params = {
            'BEHAVIOR': 4,  # Move end points only, prefer aligning nodes
            'INPUT': outputs['MergeVectorLayers_recreatedclusterlines']['OUTPUT'],
            'REFERENCE_LAYER': outputs['MergeVectorLayers_recreatedclusterlines']['OUTPUT'],
            'TOLERANCE': 10,
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['SnapGeometriesToLayer_fnlclusterlines'] = processing.run('native:snapgeometries', alg_params, context=context, feedback=feedback, is_child_algorithm=True)

        feedback.setCurrentStep(38)
        if feedback.isCanceled():
            return {}

        # Fix geometries_fnlclbndry
        alg_params = {
            'INPUT': outputs['SnapGeometriesToLayer_fnlclusterlines']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['FixGeometries_fnlclbndry'] = processing.run('native:fixgeometries', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        # Polygonize_clusters
        alg_params = {
            'INPUT': outputs['FixGeometries_fnlclbndry']['OUTPUT'],
            'KEEP_FIELDS': False,
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['Polygonize_clusters'] = processing.run('native:polygonize', alg_params, context=context, feedback=feedback, is_child_algorithm=True)

        feedback.setCurrentStep(40)
        if feedback.isCanceled():
            return {}

        # Join attributes by location_correctedclusters
        alg_params = {
            'DISCARD_NONMATCHING': False,
            'INPUT': outputs['Polygonize_clusters']['OUTPUT'],
            'JOIN': outputs['Dissolve_fnl']['OUTPUT'],
            'JOIN_FIELDS': ['struct_id'],
            'METHOD': 2,  # Take attributes of the feature with largest overlap only (one-to-one)
            'PREDICATE': [0],  # intersect
            'PREFIX': '',
        ...
        outputs['JoinAttributesByLocation_correctedclusters'] = processing.run('native:joinattributesbylocation', alg_params, context=context, feedback=feedback, is_child_algorithm=True)

        feedback.setCurrentStep(41)
        if feedback.isCanceled():
            return {}

        # Dissolve_correctedfinal
        alg_params = {
            'FIELD': ['struct_id'],
            'INPUT': outputs['JoinAttributesByLocation_correctedclusters']['OUTPUT'],
            'OUTPUT': parameters['Cluster_bndry']
        }
        outputs['Dissolve_correctedfinal'] = processing.run('native:dissolve', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        results['Cluster_bndry'] = outputs['Dissolve_correctedfinal']['OUTPUT']
        return results

    def name(self):
        return 'clusterbndry_hybrid'
